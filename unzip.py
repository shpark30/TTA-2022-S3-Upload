...
def unzip_checklist(ver, date):
    # 디렉토리 생성
    target_dir = cfg.REPORT_DIR_ORIGINAL.format(ver, date)
    if not os.path.exists(target_dir):
        os.mkdir(target_dir)
    ...
    date_columns = list(data_info.columns)
    prev_date = date_columns[date_columns.index(date)-1]
    zip_name = cfg.REPORT_DIR_ORIGINAL.format(ver, prev_date) + "_체크리스트.zip"
    with ZipFile(zip_name, 'r') as zip_obj:
        immigrated_tasks = [fn.encode('cp437').decode('utf-8') for fn in zip_obj.namelist()]

    # 이번 과제 목록 가져오기
    zip_name = cfg.REPORT_DIR_ORIGINAL.format(ver, date) + "_체크리스트.zip"
    with ZipFile(zip_name, 'r') as zip_obj:
        to_immigrate_tasks = [fn.encode('cp437').decode('utf-8') for fn in zip_obj.namelist()]
